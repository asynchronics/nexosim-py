...
def _partial_struct_unstructure_hook(e: typing.Any) -> typing.Any:  # type: ignore
    try:
        return {
            field.name: _cbor2_converter.unstructure(getattr(e, field.name))
            for field in dataclasses.fields(e)
            if getattr(e, field.name) is not None
        }
    except TypeError as err:
        raise ValueError(
            "partial struct types must expose a dataclass-like interface"
        ) from err


@typing_extensions.dataclass_transform()
def partialclass(cls: typing.Type[_T]) -> typing.Type[_T]:
    """Decorator that marks a class as a partial struct-like type.

    This automatically makes the class a dataclass. Fields with a value of
    ``None`` are omitted during serialization, matching the behavior of Rust's
    ``#[serde(skip_serializing_if = "Option::is_none")]`` attribute generated by
    the ``partially!`` macro.
    """
    cls = dataclasses.dataclass(cls)
    _cbor2_converter.register_unstructure_hook(cls, _partial_struct_unstructure_hook)
